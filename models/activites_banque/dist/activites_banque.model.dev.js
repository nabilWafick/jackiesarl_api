...
/*#__PURE__*/
function () {
  function ActivitesBanque(id, id_banque, description, debit, credit, solde_actuel, date_activite) {
  ...
  _createClass(ActivitesBanque, [{
    ...
    value: function update(callback) {
      var query = "UPDATE activites_banque SET id_banque = ?, description = ?, debit = ?, credit = ?, solde_actuel = ?, date_activite = ? WHERE id = ?";

      var id = this.id,
          updatedData = _objectWithoutProperties(this, ["id"]);

      connection.query(query, [updatedData.id_banque, updatedData.description, updatedData.debit, updatedData.credit, updatedData.solde_actuel, updatedData.date_activite, id], function (error, results) {
        if (error) {
    ...
    value: function create(activitesBanqueData, callback) {
      var query = "INSERT INTO activites_banque (id,id_banque, description, debit, credit, solde_actuel, date_activite) VALUES (NULL,?,?,?,?,?,?)";
      var currentDate = new Date();
      connection.query(query, [activitesBanqueData.id_banque, activitesBanqueData.description, activitesBanqueData.debit, activitesBanqueData.credit, activitesBanqueData.solde_actuel, currentDate], function (error, results) {
        if (error) {
          return callback(error, null);
        }
    ...
    value: function getAll(callback) {
      ...
      connection.query(query, function (error, results) {
        ...
        var activitesBanqueList = results.map(function (activitesBanqueData) {
          return new ActivitesBanque(activitesBanqueData.id, activitesBanqueData.id_banque, activitesBanqueData.description, activitesBanqueData.debit, activitesBanqueData.credit, activitesBanqueData.solde_actuel, new Date(activitesBanqueData.date_activite));
        });
