...
class Creances {
  ...
  static create(creancesData, callback) {
    const query =
      "INSERT INTO creances (id, creance_cim_benin, creance_nocibe, creance_autres, id_client,date_creance) VALUES (NULL, ?, ?, ?, ?, ?)";
    ...
    connection.query(
      ...
      (error, results) => {
        ...
        const newCreances = new Creances(
          results.insertId,
          ...Object.values(creancesData),
          currenDate
        );
  ...
  update(callback) {
    const query =
      "UPDATE creances SET creance_cim_benin = ?, creance_nocibe = ?, creance_autres = ?, date_creance = ?, id_client = ? WHERE id = ?";
