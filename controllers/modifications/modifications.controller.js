...
static getById = (req, res) => {
  ...
  Modifications.getById(id, (error, modification) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la récupération de la modification' });
    }
    if (!modification) {
...
static getAll = (req, res) => {
  Modifications.getAll((error, modifications) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la récupération des modifications' });
    }
...
static update = (req, res) => {
  ...
  Modifications.getById(id, (getError, existingModification) => {
    ...
    existingModification.update((updateError) => {
      if (updateError) {
        return res.status(500).json({ error: 'Erreur lors de la mise à jour de la modification' });
      }
