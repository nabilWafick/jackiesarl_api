...
static create = (req, res) => {
  ...
  Autorisations.create(autorisationData, (error, autorisation) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la création de l\'autorisation' });
    }
...
static getById = (req, res) => {
  ...
  Autorisations.getById(id, (error, autorisation) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la récupération de l\'autorisation' });
    }
    if (!autorisation) {
...
static getAll = (req, res) => {
  Autorisations.getAll((error, autorisations) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la récupération des autorisations' });
    }
...
static update = (req, res) => {
  ...
  Autorisations.getById(id, (getError, existingAutorisation) => {
    ...
    existingAutorisation.update((updateError) => {
      if (updateError) {
        return res.status(500).json({ error: 'Erreur lors de la mise à jour de l\'autorisation' });
      }
...
static delete = (req, res) => {
  ...
  Autorisations.getById(id, (getError, existingAutorisation) => {
    ...
    existingAutorisation.delete((deleteError) => {
      if (deleteError) {
        return res.status(500).json({ error: 'Erreur lors de la suppression de l\'autorisation' });
      }
