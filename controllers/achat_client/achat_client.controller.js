...
static create = (req, res) => {
  ...
  AchatClient.create(achatClientData, (error, achatClient) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la création de l\'achat client' });
    }
...
static getById = (req, res) => {
  ...
  AchatClient.getById(id, (error, achatClient) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la récupération de l\'achat client' });
    }
    if (!achatClient) {
...
static getAll = (req, res) => {
  AchatClient.getAll((error, achatsClients) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la récupération des achats clients' });
    }
...
static update = (req, res) => {
  ...
  AchatClient.getById(id, (getError, existingAchatClient) => {
    ...
    existingAchatClient.update((updateError) => {
      if (updateError) {
        return res.status(500).json({ error: 'Erreur lors de la mise à jour de l\'achat client' });
      }
...
static delete= (req, res) => {
  ...
  AchatClient.getById(id, (getError, existingAchatClient) => {
    ...
    existingAchatClient.delete((deleteError) => {
      if (deleteError) {
        return res.status(500).json({ error: 'Erreur lors de la suppression de l\'achat client' });
      }getById
