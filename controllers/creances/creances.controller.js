...
static create = (req, res) => {
  ...
  Creances.create(creanceData, (error, creance) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la création de la créance' });
    }
...
static getById = (req, res) => {
  ...
  Creances.getById(id, (error, creance) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la récupération de la créance' });
    }
    if (!creance) {
...
static update = (req, res) => {
  ...
  Creances.getById(id, (getError, existingCreance) => {
    ...
    existingCreance.update((updateError) => {
      if (updateError) {
        return res.status(500).json({ error: 'Erreur lors de la mise à jour de la créance' });
      }
...
static delete = (req, res) => {
  ...
  Creances.getById(id, (getError, existingCreance) => {
    ...
    existingCreance.delete((deleteError) => {
      if (deleteError) {
        return res.status(500).json({ error: 'Erreur lors de la suppression de la créance' });
      }
