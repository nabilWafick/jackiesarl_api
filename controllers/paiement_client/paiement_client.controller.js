...
static create = (req, res) => {
  ...
  PaiementClient.create(paiementClientData, (error, paiementClient) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la création du paiement client' });
    }
...
static getById = (req, res) => {
  ...
  PaiementClient.getById(id, (error, paiementClient) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la récupération du paiement client' });
    }
    if (!paiementClient) {
...
static getAll = (req, res) => {
  PaiementClient.getAll((error, paiementClients) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la récupération des paiements clients' });
    }
...
static update = (req, res) => {
  ...
  PaiementClient.getById(id, (getError, existingPaiementClient) => {
    ...
    existingPaiementClient.update((updateError) => {
      if (updateError) {
        return res.status(500).json({ error: 'Erreur lors de la mise à jour du paiement client' });
      }
...
static delete = (req, res) => {
  ...
  PaiementClient.getById(id, (getError, existingPaiementClient) => {
    ...
    existingPaiementClient.delete((deleteError) => {
      if (deleteError) {
        return res.status(500).json({ error: 'Erreur lors de la suppression du paiement client' });
      }
