...
static create = (req, res) => {
  ...
  StockBonCommande.create(stockBonCommandeData, (error, stockBonCommande) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la création du stock bon de commande' });
    }
    return res.status(201).json(stockBonCommande);
  });
};
...
static getById = (req, res) => {
  ...
  StockBonCommande.getById(id, (error, stockBonCommande) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la récupération du stock bon de commande' });
    }
    if (!stockBonCommande) {
      return res.status(404).json({ error: 'Stock bon de commande non trouvé' });
    }
...
static getAll = (req, res) => {
  StockBonCommande.getAll((error, stockBonCommandes) => {
    if (error) {
      return res.status(500).json({ error: 'Erreur lors de la récupération des stocks de bon de commande' });
    }
...
static update = (req, res) => {
  ...
  StockBonCommande.getById(id, (getError, existingStockBonCommande) => {
    ...
    existingStockBonCommande = { ...existingStockBonCommande, ...updatedData };
    existingStockBonCommande.update((updateError) => {
      if (updateError) {
        return res.status(500).json({ error: 'Erreur lors de la mise à jour du stock bon de commande' });
      }
...
static delete = (req, res) => {
  ...
  StockBonCommande.getById(id, (getError, existingStockBonCommande) => {
    ...
    existingStockBonCommande.delete((deleteError) => {
      if (deleteError) {
        return res.status(500).json({ error: 'Erreur lors de la suppression du stock bon de commande' });
      }
